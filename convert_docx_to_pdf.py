...
def convert_all_docx(input_path: str, output_path: str = None, max_workers: int = 4) -> dict:
    """
    ...
    for i, (docx_path, rel_dir, filename) in enumerate(docx_files, 1):
        ...
        if i % 10 == 0:
            logger.info(f"📈 Progresso: {i}/{len(docx_files)} | ✅ {sucessos} | ❌ {falhas} | ⏭️ {pulados}")
...
Exemplos de uso:
  ...
  python convert_docx_to_pdf.py /caminho/para/docx -w 8
    ...
    parser.add_argument(
        'input_path',
        help='Diretório onde buscar arquivos DOCX'
    )
    
    parser.add_argument(
        '-o', '--output',
        help='Diretório de saída (padrão: mesmo da entrada)'
    )
