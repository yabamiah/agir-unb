...
class S3Service:
    ...
    def download_object_by_directory(self, bucket, directory, file_path):
        try:
            ...
            if not objects_key:
                self.logger.error(
                    f"Nenhum arquivo encontrado no diretório: {directory}"
                )
                return False
            ...
            for object_key in objects_key:
                ...
                self.download_object(
                    bucket=bucket, object_name=object_key, file_path=destination_path
                )
    ...
    def listar_diretorios(self, bucket: str, prefixo: str = "") -> List[str]:
        try:
            response = self.client.list_objects_v2(
                Bucket=bucket, Prefix=prefixo, Delimiter="/"
            )
    ...
    def listar_objetos(self, bucket: str, prefixo: str = ""):
        try:
            ...
            response = self.client.list_objects_v2(Bucket=bucket, Prefix=prefixo)

            objetos = response.get("Contents", [])
            return [
                objeto["Key"] for objeto in objetos if not objeto["Key"].endswith("/")
            ]
    ...
    def object_exists(self, bucket, object_name):
        try:
            ...
            if e.response["Error"]["Code"] == "404":
                ...
                self.logger.error(
                    f"Erro ao checar o objeto: {object_name} no buclet {bucket}: {e}"
                )
                raise
