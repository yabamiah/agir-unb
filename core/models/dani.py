...
from core.processors.pdf_processor import (PDFPLUMBER_AVAILABLE,
                                           PYMUPDF_AVAILABLE,
...
class Dani:
    ...
    def __init__(
        ...
        if only_integrity_plans:
            self.docs_path = os.path.join(
                base_dir, "data", "dani", "docs", "integridade"
            )
        ...
        self.output_path = output_path or os.path.join(
            base_dir, "data", "dani", "docs", "output", "resultados.txt"
        )
        self.docx_path = docx_path or os.path.join(
            base_dir, "data", "dani", "docs", "output"
        )
        self.result_pdf = result_pdf or os.path.join(
            base_dir, "data", "dani", "docs", "result_pdf"
        )
        ...
        self.max_workers = max_workers or self.worker_manager.get_optimal_workers(
            "pdf_extraction", 10
        )
        self.pdf_workers = self.worker_manager.get_optimal_workers("pdf_extraction", 5)
        self.processing_workers = self.worker_manager.get_optimal_workers(
            "text_processing", 8
        )
        ...
        self.read_ratio = (
            read_ratio if read_ratio is not None else 0
        )  # 0 = todos os documentos
        self.params_provided = (
            all_orgaos is not None or orgao_name is not None or read_ratio is not None
        )
        ...
        if only_integrity_plans:
            ...
            dicionario_path = os.path.join(
                base_dir, "core", "motor_nlp", "dicionario.json"
            )
            try:
    ...
    def _check_conversion_dependencies(self) -> None:
        ...
        if libreoffice_available:
            self.logger.info(
                "✅ LibreOffice disponível - será usado como método principal"
            )
        else:
        ...
        if pandoc_available:
            self.logger.info(
                "✅ Pandoc disponível - será usado como método alternativo"
            )
        else:
        ...
        if not libreoffice_available and not pandoc_available:
            ...
            self.logger.error(
                "💡 Instale pelo menos uma das opções acima para converter DOCX→PDF"
            )
        else:
            self.logger.info(
                "🎉 Pelo menos uma ferramenta de conversão está disponível"
            )
    ...
    def _log_optimization_status(self) -> None:
        ...
        if PYMUPDF_AVAILABLE:
            self.logger.info(
                "✅ PyMuPDF disponível - Extração ultra-rápida para PDFs com texto"
            )
        else:
            self.logger.warning(
                "⚠️ PyMuPDF não disponível - Instale com: pip install pymupdf"
            )
        ...
        if PDFPLUMBER_AVAILABLE:
            ...
            self.logger.warning(
                "⚠️ pdfplumber não disponível - Instale com: pip install pdfplumber"
            )
    ...
    def _log_worker_status(self) -> None:
        ...
        self.logger.info(
            f"  🖥️  CPU: {system_info['cpu_physical']} cores físicos, {system_info['cpu_logical']} cores lógicos"
        )
        self.logger.info(f"  💾 Memória: {system_info['memory_gb']:.1f} GB")
        self.logger.info(
            f"  📊 Carga atual: CPU {system_info['cpu_percent']:.1f}%, Memória {system_info['memory_percent']:.1f}%"
        )
        ...
        self.logger.info("  🔧 Workers otimizados:")
        ...
        if self.worker_manager.performance_metrics:
            ...
            for task_type, metrics in self.worker_manager.performance_metrics.items():
                if metrics:
                    avg_duration = sum(m["duration"] for m in metrics[-10:]) / min(
                        10, len(metrics)
                    )
                    self.logger.info(
                        f"    {task_type}: {avg_duration:.2f}s (últimas 10 execuções)"
                    )
    ...
    def convert_single_docx_to_pdf(
        self, docx_file_path: str, pdf_output_dir: str
    ) -> bool:
        """
        ...
        try:
            ...
            try:
                ...
                if file_size == 0:
                    ...
                    self.logger.warning(
                        f"Arquivo DOCX muito pequeno (pode estar corrompido): {docx_file_path} ({file_size} bytes)"
                    )
            except OSError as e:
                self.logger.error(
                    f"Erro ao verificar arquivo DOCX: {docx_file_path} - {str(e)}"
                )
                return False
            ...
            self.logger.debug(
                f"🔄 Convertendo: {os.path.basename(docx_file_path)} → {pdf_filename}"
            )
            ...
            if success:
                ...
                if (
                    os.path.exists(pdf_output_path)
                    ...
                    self.logger.error(
                        f"❌ PDF criado mas está vazio: {pdf_output_path}"
                    )
                    return False
            else:
                self.logger.error(
                    f"❌ Falha na conversão: {os.path.basename(docx_file_path)}"
                )
                return False

        except Exception as e:
            self.logger.error(
                f"❌ Erro inesperado ao converter {docx_file_path}: {str(e)}"
            )
    ...
    def convert_orgao_docx_to_pdf(self, orgao_name: str) -> Tuple[int, int, int]:
        ...
        if not os.path.exists(orgao_docx_dir):
            self.logger.warning(
                f"Diretório não encontrado para {orgao_name}: {orgao_docx_dir}"
            )
            return (0, 0, 0)
        ...
        if invalid_files:
            self.logger.warning(
                f"⚠️ {len(invalid_files)} arquivos DOCX inválidos ignorados para {orgao_name}"
            )
        ...
        self.logger.info(
            f"📁 Convertendo {len(docx_files)} arquivos DOCX do órgão: {orgao_name}"
        )
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for docx_file in docx_files:
                ...
                future = executor.submit(
                    self.convert_single_docx_to_pdf, docx_path, orgao_pdf_dir
                )
            ...
            for future in as_completed(future_to_file):
                ...
                try:
                    ...
                    self.logger.error(
                        f"❌ Erro no processamento de {docx_file}: {str(e)}"
                    )
                    self.logger.debug(f"Traceback: {traceback.format_exc()}")
                    falhas += 1
        ...
        self.logger.info(
            f"📊 Órgão {orgao_name}: {sucessos}/{total} conversões bem-sucedidas"
        )
        if arquivos_ja_existentes > 0:
            self.logger.info(
                f"📄 {arquivos_ja_existentes} PDFs já existiam e foram pulados"
            )
    ...
    def convert_all_docx_to_pdf(self) -> Dict[str, Dict[str, int]]:
        ...
        orgaos_dirs = [
            d
            for d in os.listdir(self.docx_path)
        ...
        self.logger.info(
            f"🚀 Iniciando conversão DOCX→PDF para {len(orgaos_dirs)} órgãos"
        )
        ...
        self.logger.info("=" * 60)
        self.logger.info("📈 RESUMO DA CONVERSÃO DOCX → PDF")
        self.logger.info("=" * 60)
        ...
        self.logger.info(
            f"📊 Taxa de sucesso: {(total_sucessos / total_arquivos * 100):.1f}%"
            if total_arquivos > 0
            else "N/A"
        )
        self.logger.info("=" * 60)
        ...
        for orgao, stats in estatisticas.items():
            if stats["total"] > 0:
                self.logger.info(
                    f"🏢 {orgao}: {stats['sucessos']}/{stats['total']} ({stats['taxa_sucesso']:.1f}%)"
                )
    ...
    def _download_from_s3(self):
        ...
        """
        base = "dani-docs/"
        primeiro_nivel = self.s3_service.listar_diretorios(
            bucket="agir-bucket", prefixo=base
        )
        ...
        for prefix in primeiro_nivel:
            ...
            if nome in TIPOS_DOCUMENTO_LARA:
                for org_prefix in self.s3_service.listar_diretorios(
                    bucket="agir-bucket", prefixo=prefix
                ):
            ...
            for future in as_completed(futures):
                try:
                    if not future.result():
                        logger.warning(
                            "Não foi possível baixar alguns arquivos do bucket da S3"
                        )
                        exit(0)
                except Exception as e:
    ...
    def _setup_logger(self):
        ...
        self.logger.info(
            f"🚀 Iniciando DANI Otimizado com {self.max_workers} workers e batch_size {self.batch_size}..."
        )

    def _carregar_pelo_terminal(self):
        """Função interna para carregar keywords digitadas no terminal."""
        print("-" * 50)
        tratar_keywords = input(
    ...
    def _carregar_por_arquivo(self, caminho: str = None):
        ...
        if caminho is None:
            ...
            caminho_do_arquivo = input(
                "Digite o caminho para o arquivo .txt com as palavras-chave: "
            ).strip()
        else:
        ...
        try:
            ...
            self.logger.error(
                f"🚨 Erro: O arquivo '{caminho_do_arquivo}' não foi encontrado."
            )
            return []
        except Exception as e:
        ...
        if keywords_carregadas:
            ...
            for keyword in self.keywords.keys():
                self.compiled_patterns[keyword] = re.compile(
                    r"\b" + re.escape(keyword) + r"\w*\b", re.IGNORECASE
                )
            if not self.keywords_file:
                ...
                self.logger.info(
                    f"✅ {len(self.keywords)} palavras-chave carregadas de {self.keywords_file}"
                )
        else:
            if not self.keywords_file:
                ...
                self.logger.warning(
                    f"⚠️ Nenhuma palavra-chave carregada de {self.keywords_file}"
                )
            self.keywords = {}
    ...
    def run_with_pdf_conversion(self):
        ...
        try:
            ...
            total_time = end_time - start_time
            self.logger.info(
                f"✅ Processamento completo (incluindo PDF) concluído em {total_time:.2f} segundos!"
            )
    ...
    def catch_param(self) -> None:
        ...
        if self.params_provided:
            self.logger.info(
                "Parâmetros fornecidos via construtor, pulando entrada interativa"
            )
            if self.all_orgaos:
                self.logger.info("Modo: Processar todos os órgãos")
            else:
                self.logger.info(
                    f"Modo: Processar órgão específico - {self.orgao_name}"
                )
                self.logger.info(
                    f"Quantidade de documentos: {self.read_ratio if self.read_ratio > 0 else 'Todos'}"
                )
        else:
            ...
            if is_interactive:
                ...
                all_orgaos_input = (
                    input("Você deseja fazer uma pesquisa geral (s/n): ")
                    .strip()
                    .lower()
                )
                ...
                if not self.all_orgaos:
                    ...
                    read_ratio_input = input(
                        "Insira a quantidade de documentos que serão lidos (0 para todos): "
                    ).strip()
                    self.read_ratio = int(read_ratio_input) if read_ratio_input else 0
                else:
                    self.read_ratio = (
                        0  # Todos os documentos quando processar todos os órgãos
                    )
            else:
                ...
                self.logger.info(
                    "Configuração padrão: Processar todos os órgãos, todos os documentos"
                )
    ...
    def read_docs_parallel_batched(self) -> None:
        ...
        self.logger.info(
            f"📚 Processando {len(files_to_process)} arquivos em lotes de {self.batch_size}"
        )
    ...
    def read_all_docs_parallel_batched(self) -> None:
        ...
        all_dir = [
            join(self.docs_path, d)
            for d in listdir(self.docs_path)
            if isdir(join(self.docs_path, d))
        ]
        ...
        if self.only_integrity_plans:
            ...
            read_orgaos = [
                os.path.basename(d)
                for d in listdir(self.docx_path)
                if isdir(join(self.docx_path, d))
            ]
            orgaos_to_read = [
                orgao for orgao in all_dir if os.path.basename(orgao) not in read_orgaos
            ]

        self.logger.info(
            f"Órgãos a serem lidos: {[os.path.basename(o) for o in orgaos_to_read]}"
        )
        ...
        self.logger.info(
            f"📚 Processando {len(all_files)} arquivos em lotes de {self.batch_size}"
        )
        ...
        for orgao_name, files in orgao_files.items():
            self.logger.info(
                f"🏢 Processando órgão: {orgao_name} ({len(files)} arquivos)"
            )
    ...
    def _process_batch(self, batch: List[str], docs_path: str, orgao_name: str) -> None:
        ...
        with executor as exec:
            ...
            for file in batch:
                ...
                future = exec.submit(
                    self._process_single_pdf_safe, file_path, orgao_name, file
                )
    ...
    def _process_batch_all_orgaos(self, batch: List[Tuple[str, str, str]]) -> None:
        ...
        with executor as exec:
            ...
            for file_path, orgao_name, filename in batch:
                future = exec.submit(
                    self._process_single_pdf_safe, file_path, orgao_name, filename
                )
                futures.append(future)
    ...
    def _process_single_pdf_safe(
        self, file_path: str, orgao_name: str, filename: str
    ) -> Optional[Dict]:
    ...
    def _process_single_pdf(
        ...
        try:
            ...
            if len(pdf_pages_text) > 1:
                ...
                executor = self.worker_manager.get_executor(
                    "text_processing", len(pdf_pages_text)
                )
                ...
                with executor as exec:
                    ...
                    for i, page_text in enumerate(pdf_pages_text):
                        if page_text:
                            future = exec.submit(
                                self._process_page_text, page_text, filename, orgao_name
                            )
                            futures.append(future)
                ...
                for page_text in pdf_pages_text:
                    ...
                    page_keyword_counts, snippets = self._process_page_text(
                        page_text, filename, orgao_name
                    )
            ...
            imga_result = None
            if (
                self.only_integrity_plans
                and self.classificador
                and self.pontuador
                and self.calculador
            ):
                ...
                for page_text in pdf_pages_text:
                    ...
                    for segmento in segmentos:
                        ...
                        self.classificador.classificar_texto(segmento)

                        # Para cada eixo, verificar pontuação neste segmento
                        for (
                            eixo_id,
                            sinais_config,
                        ) in self.classificador.dicionario_eixos.items():
                            resultado = self.pontuador.calcular_score_eixo(
                                segmento, eixo_id, sinais_config
                            )
                            if resultado.score_final > 0:
                                scores_por_eixo[eixo_id].append(resultado.score_final)
                                evidencias_por_eixo[eixo_id].extend(
                                ...
                                for (
                                    booster_id,
                                    ativado,
                                ) in resultado.detalhe_boosters.items():
                                    if ativado:
                ...
                imga_result = self.calculador.calcular_imga_entidade(
                    scores_por_eixo, total_words_doc
                )
                ...
                with self.snippet_lock:
                    if self.imga_results is not None:
                        ...
                        self.logger.info(
                            f"📊 IMGA calculado para {filename}: Global={imga_result['imga_global']}, Faixa={imga_result['faixa']}"
                        )
    ...
    def _process_page_text(
        self, page_text: str, filename: str, orgao_name: str
            ...
            if combined_pattern:
                ...
                for keyword, keyword_match_list in keyword_matches.items():
                    ...
                    for match in keyword_match_list:
                        snippet_text = self._extract_snippet_optimized(
    ...
    def _extract_snippet_optimized(
        self, text: str, start_pos: int, end_pos: int, window_size: int = 300
    ) -> str:
                ...
                for existing_snippet in recent_snippets:
                    try:
                        similarity = self._calculate_similarity_simple(
                            snippet_text, existing_snippet["text"]
                        )
                        if similarity >= 0.85:
                            is_duplicate = True
                            break
    ...
    def _calculate_similarity_cached(self, text1: str, text2: str) -> float:
        ...
        cache_key = (
            f"{self._calculate_hash(text1)[:8]}_{self._calculate_hash(text2)[:8]}"
        )
            ...
            with self.file_semaphore:
                with self.snippet_lock:
                    ...
                    if key not in self.docx_files:
                        self.docx_files[key] = self.create_keyword_docx(
                            keyword, orgao_name
                        )
                    ...
                    snippet_fixed = "".join(
                        c if c.isprintable() else " " for c in snippet_with_file
                    )
    ...
    def save_docs_for_current_orgao(self, orgao_name: str) -> None:
        ...
        try:
            ...
            os.makedirs(org_dir, exist_ok=True)
            docs_to_save = {
                k: v for k, v in self.docx_files.items() if k[0] == orgao_name
            }
    ...
    def upload_docx_s3(self):
        ...
        try:
            ...
            if not os.path.exists(docx_path):
                logger.error(
                    "❌ Falha ao enviar docxs para o S3. DOCX não encontrados."
                )
                return False
            ...
            for entry in entries:
                ...
                if os.path.isdir(orgao_path):
                    ...
                    for docx in docxs:
                        ...
                        try:
                            if not self.s3_service.object_exists(
                            ...
                            logger.error(
                                f"Erro ao verificar existência do arquivo na S3: {e}"
                            )
        ...
        for orgao, docs in docs_por_orgao.items():
            ...
            for keyword, total_count in self.keywords.items():
                if len(self.name_docs_read) > 0:
                    orgao_count = int(
                        (total_count * len(docs)) / len(self.name_docs_read)
                    )
            ...
            for keyword, count in orgao_keywords.items():
                percentage = (
                    (count / total_orgao_words) * 100.0 if total_orgao_words > 0 else 0
                )
                orgao_percentages[keyword] = round(percentage, 2)

            # Top keywords do órgão
            top_keywords = sorted(
                orgao_keywords.items(), key=lambda x: x[1], reverse=True
            )[:10]
            ...
            orgao_stats[orgao] = {
                ...
                "media_ocorrencias_por_documento": (
                    round(sum(orgao_keywords.values()) / len(docs), 2) if docs else 0
                ),
            }
    ...
    def display_results(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Órgão(s) analisado(s): {', '.join(self.orgaos_processados)}"
            )
            self.logger.info(f"Documentos lidos: {len(self.name_docs_read)}")
            self.logger.info(
                "Quantidade de vezes que cada palavra-chave foi encontrada:"
            )
            ...
            if self.only_integrity_plans and self.imga_results:
                ...
                for filename, result in self.imga_results.items():
                    self.logger.info(f"\n📄 Documento: {filename}")
                    self.logger.info(
                        f"   🏆 IMGA Global: {result['imga_global']} ({result['faixa']})"
                    )
                    self.logger.info("   📊 Índices por Eixo:")
                    for eixo, score in result["indices_eixos"].items():
                        nome_eixo = (
                            self.classificador.obter_info_eixo(eixo)
            ...
            summary = {
                "metadata": {
                    ...
                    "tipo_analise": (
                        "integridade"
                        if self.only_integrity_plans
                        else ("geral" if self.all_orgaos else "especifica")
                    ),
                    "orgao_especifico": (
                        self.orgao_name if not self.all_orgaos else None
                    ),
                },
                "resumo_geral": {
                    ...
                    "media_ocorrencias_por_documento": (
                        round(sum(self.keywords.values()) / len(self.name_docs_read), 2)
                        if self.name_docs_read
                        else 0
                    ),
                },
            ...
            for keyword, count in self.keywords.items():
                percentage = (
                    (count / self.total_words_read) * 100.0
                    if self.total_words_read > 0
                    else 0
                )
            ...
            # Top keywords gerais
            top_keywords_geral = sorted(
                self.keywords.items(), key=lambda x: x[1], reverse=True
            )[:20]
            summary["top_keywords_geral"] = [
                {
                    "keyword": k,
                    "count": c,
                    "percentage": summary["porcentagem_keywords_geral"][k],
                }
                for k, c in top_keywords_geral
            ]
            ...
            for orgao, stats in orgao_statistics.items():
                ...
                self.logger.info(
                    f"   📝 Palavras lidas: {stats['total_palavras_lidas']:,}"
                )
                self.logger.info(
                    f"   🔍 Total ocorrências: {stats['total_ocorrencias']}"
                )
                self.logger.info(
                    f"   📊 Média por documento: {stats['media_ocorrencias_por_documento']}"
                )

                if stats["top_keywords"]:
                    top_3 = stats["top_keywords"][:3]
                    keywords_str = ", ".join(
                        [f"{k['keyword']}({k['count']})" for k in top_3]
                    )
                    self.logger.info(f"   🏆 Top keywords: {keywords_str}")

            self.logger.info(
                "No diretório de output você pode verificar os pedaços em que o texto foi encontrado"
            )

            try:
                summary_path = os.path.join(
                    os.path.dirname(self.output_path), "..", "summary_results.json"
                )
                os.makedirs(os.path.dirname(summary_path), exist_ok=True)
                with open(summary_path, "w", encoding="utf-8") as f:
                    json.dump(summary, f, ensure_ascii=False, indent=4)
                self.logger.info(
                    f"Sumário detalhado dos resultados salvo em: {summary_path}"
                )
            except Exception as e:
