...
def compare_maps(data_map: dict[str, bool], links_map: dict[str, list[str]]) -> tuple[dict[str, bool], int, int, int]:
...
def check_keywords(link: str, keywords: str) -> bool:
...
def load_orgaos_name(direct_admin: list, mixed_public: list) -> int:
    ...
    for line in file:
        ...
        if not is_mista:
            direct_admin.append(line.strip())
        else:
            mixed_public.append(line.strip())
...
def extract_links(soup: BeautifulSoup, acronym: str, org: str, limit: int):
    """
    ...
    for g in soup.find_all('div',  {'class': 'Gx5Zad'}):
        ...
        for link in found_links:
            ...
            if ("comitê interno de governança" in link_text or
                ...
                if acronym.lower() in link_href and "df" in link_href or check_keywords(link_href, org):
                    links.append(link)

    return links


def save_links_to_json(links_dict: dict[str, list[str]], filename: str) -> None:
...
def load_links_from_json(filename: str) -> dict[str, list[str]]:
...
class Lara:
    ...
    # TODO: Ajustar a extracao para verificar se o link encontrado possui a sigla ou nome do órgão
    # TODO: Verificar o log gerado para verificar o que falta pegar
    def search_link_cig(self, acronym: str, org: str, param: str, base_link: str, limit: int) -> dict[str, list[str]]:
        ...
        org_name_params = [acronym + " df", acronym, org]
        org_link_dict = {}
        ...
        try:
            for org_name_param in org_name_params:
                ...
                links = extract_links(soup, acronym, org, limit)

                for link in links:
                    link_addrs = link['href']

                    print("Org:", org, " Link:", link_addrs)
                    if acronym.lower() in link_addrs.lower() or org.lower() in link_addrs.lower():
                        print("Org:", org, " Link:", link_addrs)
            ...
            if response.status_code == 200:
             ...
             # Usando BeautifulSoup para uma análise mais robusta do HTML               return True, True, None
                    ...
                    for link in links:
                        ...
                        is_ata_cig = (check_keywords(link_href, "comite") and check_keywords(
                            link_href, "interno") and check_keywords(link_href, "governanca"))
    ...
    def generate_docx_report(self, content: list[str]) -> None:
        """Generate a report with the collected links by organizations and the number of results."""
    ...
    def generate_pdf_report(self) -> None:
        ...
        self.document_service.convert_docx_to_pdf(input_file=self.path_docx_report)
    ...
    def execute(self) -> None:
        ...
        excel = "data/FAPDF-Segmentoseitensintegridade(versão20.04.Completo)_corrigido.xlsx"
        dados = read_excel_data(excel)
        if dados is None:
        ...
        dict_general = {}
        links_json = None
        links_json = load_links_from_json('data/lara/json_map_links.json')
        
        if not links_json:
            for orgao in adm_direta:
                dict_general.update(self.search_link_cig(company_acronym(orgao), orgao,
                                                         param, url_google, 10))
                time.sleep(3)
                
            save_links_to_json(dict_general, 'data/lara/json_map_links.json')
        else:
            dict_general = links_json    

        print(dict_general)
        exit(0)
        resul, acertos, total_com_ata, total_sem_ata = compare_maps(dados, dict_general)
        ...
        # Gerando a lista com os órgãos e se possuem atas
        orgao_details = [f"{orgao} tem atas: {'Sim' if tem_atas else 'Não'}" for orgao, tem_atas in resul.items()]
        report_content.extend(orgao_details)
    ...
    def donwload_minutes(self, orgao: Orgao ) -> None:
        ...
        try:
            ...
            diretorio = f'data/lara/docs/{orgao.name}'
            ...
            for link in links:
                ...
                for pdf_link in minutes_links:
                    file_name = diretorio + f"/{orgao.name}_ata{file_counter}.pdf"
