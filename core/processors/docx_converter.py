...
class DocxConverter:
    ...
    def _convert_with_libreoffice(
        self, docx_file_path: str, pdf_output_path: str
    ) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if os.path.exists(expected_pdf):
                    ...
                    self.logger.info(
                        f"✅ [LibreOffice] Convertido: {os.path.basename(docx_file_path)}"
                    )
                    return True
                else:
                    self.logger.error(
                        f"❌ [LibreOffice] PDF não foi criado: {expected_pdf}"
                    )
                    return False
            else:
                self.logger.error(
                    f"❌ [LibreOffice] Erro na conversão: {result.stderr}"
                )
                return False

        except subprocess.TimeoutExpired:
            self.logger.error(
                f"❌ [LibreOffice] Timeout na conversão de {docx_file_path}"
            )
            return False
        ...
        for engine in engines:
            try:
                ...
                if (
                    ...
                    self.logger.info(
                        f"✅ [pypandoc-{engine}] Convertido: {os.path.basename(docx_file_path)}"
                    )
                    return True
                else:
                    self.logger.warning(
                        f"⚠️ [pypandoc-{engine}] PDF vazio ou não criado"
                    )
    ...
    def docx_to_pdf(self, docx_file_path: str, pdf_output_path: str) -> bool:
        ...
        try:
            ...
            self.logger.debug(
                f"🔄 Iniciando conversão: {os.path.basename(docx_file_path)}"
            )
            ...
            self.logger.error(
                f"❌ Todos os métodos de conversão falharam para: {os.path.basename(docx_file_path)}"
            )
            return False

        except Exception as e:
            self.logger.error(
                f"❌ Erro crítico na conversão de {docx_file_path}: {str(e)}"
            )
            return False
        ...
        try:
            ...
            try:
                ...
                if file_size == 0:
                    ...
                    self.logger.warning(
                        f"Arquivo DOCX muito pequeno: {docx_file_path} ({file_size} bytes)"
                    )
            except OSError as e:
                self.logger.error(
    ...
    def convert_orgao_docx(
        self, docx_dir: str, pdf_dir: str, orgao_name: str
    ) -> Tuple[int, int, int]:
        """
        ...
        if not os.path.exists(docx_dir):
            self.logger.warning(
                f"Diretório não encontrado para {orgao_name}: {docx_dir}"
            )
            return (0, 0, 0)
        ...
        docx_files = [
            f
            for f in all_files
            if f.endswith(".docx") and os.path.getsize(os.path.join(docx_dir, f)) > 0
        ]
    ...
    def convert_all_orgaos(
        self, docx_base_path: str, pdf_base_path: str
        ...
        orgaos_dirs = [
            d
            for d in os.listdir(docx_base_path)
